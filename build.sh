...
CERT="Developer ID Application: Your Name (xxxxxxxxxx)"
# This is your login for the apple developer site
EMAIL="david.deng0822@gmail.com"
...
UUID=$(xcrun altool -t osx -f "${APP_NAME}".dmg --primary-bundle-id "${APP_ID}" --notarize-app --username "${EMAIL}" -p "${PASSWORD}" |grep RequestUUID | awk '{print $NF}')
rm -fr "${APP_NAME}/"
...
while true; do
        sleep 5
        echo checking....
        xcrun altool --notarization-info "${UUID}" -p "${PASSWORD}" --username "${EMAIL}" | grep -s Approved && break
done
